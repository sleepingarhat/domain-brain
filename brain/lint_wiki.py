...
SKIP_SECTION_TYPES = {"index", "log", "meta", "hot"}
...
def lint_wiki() -> dict:
    ...
    required = {"AGENTS.md", "wiki/RULES.md", "wiki/routing-map.md", "wiki/hot.md"}
    root = WIKI_DIR.parent
    for rel in required:
        if not (root / rel).exists():
            issues.append(f"缺 {rel}")
    ...
    for page in pages:
        ...
        if key in titles and ptype not in ("index", "log", "style", "meta", "hot"):
            issues.append(f"{rel}: 同類型重複 title {page.title}")
        else:
            titles[key] = rel
        if ptype not in SKIP_SECTION_TYPES:
            for sec in SECTIONS:
